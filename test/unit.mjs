...
const {
  ...
  getOverrideWarnings,
  switchOrganization,
} = require('../plugin/hooks/switch-org.js');
...
describe('organization switching', () => {
  test('browser auth uses an ephemeral, state-protected callback', async () => {
    ...
    const auth = startAuthFlow({
      ...
      openUrl: (authUrl) => {
        browserFlow = (async () => {
          ...
          assert.match(callback.searchParams.get('state'), /^[a-f0-9]{64}$/);

          const forgedCallback = new URL(callback);
          forgedCallback.searchParams.set('state', 'incorrect-state');
          forgedCallback.searchParams.set('apikey', candidateKey);
          const forgedResponse = createResponse();
          callbackHandler(
            { url: `${forgedCallback.pathname}${forgedCallback.search}` },
            forgedResponse,
          );
          assert.equal(forgedResponse.statusCode, 400);

          callback.searchParams.set('apikey', candidateKey);
    ...
    assert.equal(await auth, candidateKey);
    await browserFlow;
    assert.equal(listenPort, 0);
  });

  test('does not replace credentials when candidate verification fails', async () => {
    let persisted = false;
    await assert.rejects(
      switchOrganization({
        ...
        projectConfig: null,
        env: {},
        authenticate: async () => 'sm_unverified_0123456789abcdef',
        verify: async () => {
  ...
  test('verifies the selected organization before saving and reports overrides', async () => {
    const candidateKey = 'sm_new_org_0123456789abcdef';
    let sessionRequest = null;
    let persistedKey = null;

    const result = await switchOrganization({
      baseUrl: 'https://api.example.test/',
      projectConfig: { apiKey: 'sm_project_override' },
      env: { SUPERMEMORY_CC_API_KEY: 'sm_env_override' },
      authenticate: async () => candidateKey,
      fetch: async (url, init) => {
        sessionRequest = { url, init };
        return {
          ok: true,
          json: async () => ({
            org: { id: 'org_new', name: 'New Organization', slug: 'new-org' },
          }),
        };
      },
      persist: (apiKey) => {
    ...
    assert.equal(persistedKey, candidateKey);
    ...
    assert.equal(sessionRequest.url, 'https://api.example.test/v3/session');
    assert.equal(
      sessionRequest.init.headers.Authorization,
      `Bearer ${candidateKey}`,
    );
  });

  test('rejects session responses without an organization identity', () => {
    assert.throws(
      () => getOrganization({ org: { name: 'Missing ID' } }),
      /incomplete/,
    );
    assert.equal(getOverrideWarnings({}, null).length, 0);
  });
});
